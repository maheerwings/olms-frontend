...
const Catalog = () => {
  ...
  const { allBorrowedBooks, error, message, loading } = useSelector(
    (state) => state.borrow
  );
  const { returnBookPopup } = useSelector((state) => state.popup);

  const [filter, setFilter] = useState("borrowed"); // "borrowed" or "overdue"
                ...
                {booksToDisplay.map((book, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2">
                      {formatOnlyDate(book.dueDate)}
                    </td>
                    <td className="px-4 py-2">
                      {formatDateAndTime(book.createdAt)}
                    </td>
                    <td className="px-4 py-2 flex space-x-2 my-3">
                      {book.returnDate ? (
